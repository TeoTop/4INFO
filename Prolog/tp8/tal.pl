...
nom_commun_arbre([X|L],L,nc(X),G,N):-nc(X,G,N).
nom_propre_arbre([X|L],L,np(X),G,N):-np(X,G,N).
adjectif_arbre([X|L],L,adj(X),G,N):-adj(X,G,N).
prep_arbre([X|L],L,prep(X)):-pre(X).
verbe_arbre([X|L],L,vb(X),N):-vb(X,N).
pronom_arbre([X|L],L,pn(X)):-pn(X).
...
/* Question 2.3 */
analyse_arbre(L,A):-phrase_simple_arbre(L,[],A).
phrase_simple_arbre(L,R,phr(A1,A2)):-gp_nominal_arbre(L,L1,A1,G,N),gp_verbal_arbre(L1,R,A2,G,N).

gp_nominal_arbre(L,R,gn(A1,A2),G,N):-article_arbre(L,L1,A1,G,N),nom_commun_arbre(L1,R,A2,G,N).

gp_nominal_arbre(L,R,gn(A1,A2,A3),G,N):-article_arbre(L,L1,A1,G,N),nom_commun_arbre(L1,L2,A2),relatif_arbre(L2,R,A3,N).
